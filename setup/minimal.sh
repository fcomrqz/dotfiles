...
gum spin --title "fish: Finally, a command line shell for the 90s" brew install fish && mkdir -p ~/.config/fish && ln -sF ~/Developer/fcomrqz/dotfiles/shells/fish/functions ~/.config/fish && ln -sF ~/Developer/fcomrqz/dotfiles/shells/fish/config.fish ~/.config/fish/config.fish
...
gum spin --title "bun: JavaScript Runtime built on Apple JavaScriptCore" curl -fsSL https://bun.sh/install | bash
gum spin --title "git: The stupid content tracker" brew install git && ln -sF ~/Developer/fcomrqz/dotfiles/git/.gitconfig ~/.gitconfig
gum spin --title "gh: GitHub CLI" brew install gh && mkdir ~/.config/gh && ln -sF ~/Developer/fcomrqz/dotfiles/gh/config.yml ~/.config/gh/
gum spin --title "gitui: git graphical interface" brew install gitui && mkdir ~/.config/gitui && ln -sF ~/Developer/fcomrqz/dotfiles/gitui/theme.ron ~/.config/gitui/theme.ron
gum spin --title "delta: A better diff" brew install git-delta
gum spin --title "tree: Display directories as trees" brew install tree
